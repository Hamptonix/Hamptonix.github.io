...
// Firebase reference for prints
const printsRef = ref(db, 'prints');

// Global canvas reset every hour
...
setInterval(() => {
  ...
  if (timeLeft <= 0) return; // we'll trigger reset below
  ...
  resetCountdownEl.textContent =
    `Resetting in: ${String(hours).padStart(2, '0')}:${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
}, 1000);

// Global reset logic — only one client (or backend) should trigger it to avoid conflicts
setInterval(() => {
  const now = new Date();
  if (now.getMinutes() === 0 && now.getSeconds() < 3) {
    // Only reset once per hour (during first 3 seconds of hour)
    const resetKey = `reset_${now.getHours()}`; // Prevent multiple clears per hour

    if (!window._lastReset || window._lastReset !== resetKey) {
      window._lastReset = resetKey;

      // Clear all prints
      set(printsRef, null)
        .then(() => console.log('Canvas reset at top of the hour'))
        .catch(console.error);
    }
  }
}, 1000);
